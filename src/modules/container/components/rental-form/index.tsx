...
import { FormInputs } from "../../lib/useCreateContainer";
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col items-center justify-center">
      <div className="flex flex-col lg:flex-row items-start justify-between w-full mb-6">
        <Label className="text-lg font-bold" htmlFor="rental-info">Informacion del Contenedor</Label>
        <div className="w-full lg:w-auto flex flex-wrap justify-between">
          <div className="lg:ml-4">
            ...
            <div className="mb-3">
              <Label htmlFor="status">Almacen</Label>
              <select {...register('store_id')} >
                <option value="">Eliga un almacen..</option>
                {stores.map((shop: Store, index: number) => (
                  <option key={`shop-${index}`} value={shop.id}>
                    {shop.name}
                  </option>
                ))}
              </select>
              {errors.store_id && <ErrorInput>{errors.store_id.message}</ErrorInput>}
            </div>
          </div>
          <div className="lg:ml-4">
            <div className="mb-3">
            ...
            <div className="mb-3">
              <Label htmlFor="status">Equipo</Label>
            ...
            <div className="mb-3">
              <Label htmlFor="status">Projecto</Label>
              <select {...register('project_id')} >
                <option value="">Eliga un equipo..</option>
                {projects.map((shop, index) => (
                  <option key={`shop-${index}`} value={shop.id}>
                    {shop.name}
                  </option>
                ))}
