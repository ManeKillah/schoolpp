...
export const columns: ColumnDef<RentalExtended>[] = [
  {
    header: ({column}) => {
  ...
  // },
  {
    ...
    cell: ({row}) => {
      ...
      <div className="flex items-center gap-2">
        <Link className="text-warning" href={`/inicio/rental/${row.getValue('id')}`} style={{ marginRight: '12px' }}>
          <EyeIcon className="size-4" />
        </Link>
        <Link className="text-warning" href={`/inicio/rental/${row.getValue('id')}/details`}>
